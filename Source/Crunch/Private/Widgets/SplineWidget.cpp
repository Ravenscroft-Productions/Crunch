...
void USplineWidget::SetSplineStyle(const FLinearColor InColour, float InThickness)
{
	Colour = InColour;
...
int32 USplineWidget::NativePaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	FSlateDrawElement::MakeSpline(OutDrawElements, ++LayerId, AllottedGeometry.ToPaintGeometry(), StartPos, StartPortDirection, EndPos, EndPortDirection, Thickness, ESlateDrawEffect::None, Colour);
	return LayerId;
}
