...
void UCAbilitySystemComponent::HealthUpdated(const FOnAttributeChangeData& ChangeData)
{
	...
	if (ChangeData.NewValue <= 0.0f)
	{
		if (!HasMatchingGameplayTag(UCAbilitySystemStatics::GetHealthEmptyStatTag()))
		{
			...
			if (AbilitySystemGenerics && AbilitySystemGenerics->GetDeathEffect()) AuthApplyGameplayEffect(AbilitySystemGenerics->GetDeathEffect());
...
void UCAbilitySystemComponent::ExperienceUpdated(const FOnAttributeChangeData& ChangeData)
{
	...
	float PrevLevelExp = 0.0f;
	float NextLevelExp = 0.0f;
	float NewLevel = 1;
